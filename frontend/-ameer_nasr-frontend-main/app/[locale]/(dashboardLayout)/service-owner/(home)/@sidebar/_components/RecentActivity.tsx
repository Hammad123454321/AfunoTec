...
function ActivityItem({ activity }: { activity: Activity }) {
  ...
  return (
    <div className="flex gap-4 group hover:bg-gray-100 p-2 rounded">
      <div
        className={cn(
          "size-10 rounded-full flex items-center justify-center flex-shrink-0 text-white",
          iconBg
        )}
      >
        {icon}
...
export function RecentActivity({
  ...
  if (data.length === 0) {
    return (
      <div className={cn("bg-white rounded shadow-sm p-6", className)}>
  ...
  return (
    <div className={cn("bg-white rounded shadow-sm p-2 space-y-8", className)}>
      <Heading as="h3" size="h6">
        {title}
      </Heading>
...
const ExampleUsage: React.FC = () => {
  ...
  return (
    <RecentActivity title="Recent Activity" data={data} isLoading={loading} />
  );
};
