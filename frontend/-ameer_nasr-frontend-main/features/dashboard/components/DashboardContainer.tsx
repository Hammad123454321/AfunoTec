...
export default function DashboardContainer({
  ...
  return (
    <div
      ...
      className={cn(
        `grid transition-[grid] duration-200 bg-gray-200 min-h-screen`
      )}
    >
      {children}
    </div>
...
export function DashboardSidebarContainer({ children }: DashboardSidebarProps) {
  return (
    <div className="[grid-area:sidebar] print:hidden h-screen sticky top-0">
      {children}
    </div>
  );
}
...
export function DashboardHeaderContainer({ children }: DashboardSidebarProps) {
  return (
    <div className="[grid-area:header] sticky top-0 z-50 bg-white border-b border-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.1]">
      {children}
    </div>
  );
}
...
export function DashboardMainContainer({ children }: DashboardMainProps) {
  return <div className="[grid-area:main] overflow-x-auto">{children}</div>;
}
